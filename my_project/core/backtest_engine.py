...
class BacktestEngine:
    ...
    def __init__(self, event_engine: EventEngine, data_manager: DataManager):
        ...
        # 修改处1：添加内部账户状态维护，解决数据同步时序问题
        self.internal_account_state = {
            "balance": 1000000.0,
            "available": 1000000.0,
            "commission": 0.0,
            "margin": 0.0,
        ...
        # 修改处2：添加内部交易记录，确保报告数据一致性
        self.internal_trade_records = []
        self.internal_position_records = []
    ...
    async def run_backtest(self, strategy_name: str, strategy_config: Dict[str, Any] = None):
        ...
        with thread_safe_manager.locked_resource("backtest_execution"):
            ...
            # 修改处4：确保最终数据同步在报告生成前完成
            if not self._final_sync_performed:
                await self._perform_final_sync_with_validation()  # 修改为异步并添加验证
    ...
    def _create_trade(self, symbol: str, direction: str, price: float, volume: int,
        ...
        if trade_table:
            ...
            try:
                ...
                self.internal_trade_records.append(internal_trade)
    ...
    def _update_position(self, symbol: str, direction: str, price: float, volume: int, trade_id: str):
        ...
        if position_table:
            ...
            try:
                ...
                self.internal_position_records.append(internal_position)
    ...
    def _update_account(self, direction: str, price: float, volume: int, commission: float):
        """更新账户"""
        # 修改处7：同时更新内部账户状态和外部数据表
        self.internal_account_state["balance"] -= commission
        self.internal_account_state["available"] -= commission
        self.internal_account_state["commission"] += commission
    ...
    def _perform_data_sync(self):
        """执行数据同步检查"""
        try:
            # 修改处8：增强数据同步检查，验证内部状态与外部数据一致性
            internal_balance = self.internal_account_state.get("balance", 0)
            account_table = self.data_manager.get_table("account")
            if account_table:
                external_account = account_table.get_account() if hasattr(account_table, 'get_account') else {}
                external_balance = external_account.get("balance", 0)

                consistency = abs(internal_balance - external_balance) < 0.01
                print(f"数据同步完成: 一致性={consistency}, 内部余额={internal_balance}, 外部余额={external_balance}")
            else:
                print("数据同步完成: 一致性=False (账户表不存在)")
        except Exception as e:
            print(f"[{datetime.now()}] [BacktestEngine] 数据同步失败: {e}")

    async def _perform_final_sync_with_validation(self):
        """执行最终数据同步（增强版本，带验证）"""
        try:
            ...
            for attempt in range(max_retries):
                ...
                if account_table:
                    ...
                    if abs(internal_balance - external_balance) < 0.01:
                        print(f"[{datetime.now()}] [BacktestEngine] 数据一致性验证通过")
                        break
                    else:
                        print(f"[{datetime.now()}] [BacktestEngine] 数据不一致，第{attempt+1}次重试...")
                        if attempt < max_retries - 1:
                            await asyncio.sleep(0.1)
                else:
                    print(f"[{datetime.now()}] [BacktestEngine] 账户表不存在，跳过一致性检查")
                    break
    ...
    def _generate_backtest_report(self) -> Dict[str, Any]:
        """生成回测报告（修复：使用内部状态确保数据一致性）"""
        try:
            # 修改处10：使用内部状态生成报告，避免时序问题
            account_data = self.internal_account_state
            ...
            report = {
                ...
                "trade_statistics": {
                    "total_trades": len(self.internal_trade_records),
                    "trade_records": self.internal_trade_records
                },
                "position_statistics": {
                    "total_positions": len(self.internal_position_records),
                    "position_records": self.internal_position_records
                },
                "error_statistics": {
                    "total_errors": self.backtest_stats.get("total_errors", 0),
                    "error_rate": round(self.backtest_stats.get("total_errors", 0) / max(len(self.history_data), 1), 4)
                },
                "summary": "回测报告生成完成（使用内部状态确保一致性）"
            }
    ...
    def _print_report_summary(self, report: Dict[str, Any]):
        ...
        print(f"总交易次数: {trades.get('total_trades', 0)} (内部记录: {metrics.get('total_trades', 0)})")
    ...
    def get_backtest_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "internal_account_state": self.internal_account_state
        }
